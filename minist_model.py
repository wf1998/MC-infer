...
class TinyMinistModel(keras.Model):
    def __init__(self):
        super(TinyMinistModel, self).__init__()
        self.conv1 = layers.Conv2D(16, kernel_size=3, strides=2, input_shape=(28, 28, 1), padding="same")
        self.drop = layers.Dropout(0.25)
...
class SubstituteModel_small(keras.Model):
    def __init__(self):
        super(SubstituteModel_small, self).__init__()
        self.conv1 = layers.Conv2D(16, kernel_size=3, strides=2, input_shape=(28, 28, 1), padding="same")
        self.drop = layers.Dropout(0.25)
...
class SubstituteModel_large(keras.Model):
    def __init__(self):
        super(SubstituteModel_large, self).__init__()
        self.conv1 = layers.Conv2D(16, kernel_size=3, strides=2, input_shape=(28, 28, 1), padding="same")
        self.drop = layers.Dropout(0.25)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        x = self.drop3(tf.nn.leaky_relu(self.conv4(x), alpha=0.2))
        x = self.bn3(self.drop4(tf.nn.leaky_relu(self.conv5(x), alpha=0.2)))
        x = self.bn4(self.drop5(tf.nn.leaky_relu(self.conv6(x), alpha=0.2)))
        x = self.gap(x)
        category = tf.nn.softmax(self.fc3(x))
        return category
